(* Bit stream module
 * Copyright, Martin Elsman 2003-2018
 * MIT Licence
 *)
...
structure Bytestream :> BITSTREAM =
  struct
    ...
    type outstream =
         {pos: pos,               (* bytes written *)
          buf: Word8Array.array   (* output *)
         }

    fun isEmpty {pos,buf} =
        pos >= Word.fromInt(Word8Vector.length buf)
    ...
    fun array_capacity buf =
        Word.fromInt (Word8Array.length buf)

    fun vector_capacity buf =
        Word.fromInt (Word8Vector.length buf)

    fun ensurespace (s as {pos,buf}) =
        if pos + 0w1 < array_capacity buf then s
        else {pos=pos,buf=double buf}

    fun outwN n (e : Word8.word, s:outstream) : outstream =   (* n \in {1,2,3,4,5,6,7,8} *)
        if n > 0w8 then raise Fail "outwN"
        else let val s as {pos,buf} = ensurespace s
             in Word8Array.update(buf,Word.toIntX pos,e)
              ; {pos=pos+0w1, buf=buf}
             end

    fun getwN n ({pos,buf}:instream) : Word8.word * instream = (* n \in {1,2,3,4,5,6,7,8} *)
        if n > 0w8 then raise Fail "getwN"
        else if pos + 0w1 > vector_capacity buf then
          raise Fail "getwN: empty in-stream"
        else (Word8Vector.sub(buf,Word.toIntX pos), {pos=pos+0w1,buf=buf})
    ...
    fun toBytes {pos,buf} =
        let val slice = Word8ArraySlice.slice(buf,0,SOME (Word.toIntX pos + 1))
